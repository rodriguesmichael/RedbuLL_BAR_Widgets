...
function widget:Initialize()
	...
	if not WG['chat_filtered'] then
		WG['chat_filtered'] = {}
	end
	WG['chat_filtered'].isInputActive = function()
		return showTextInput
	end
	WG['chat_filtered'].getInputButton = function()
		return inputButtonRect
	end
	WG['chat_filtered'].setHide = function(value)
		hide = value
	end
	WG['chat_filtered'].getHide = function()
		return hide
	end
	WG['chat_filtered'].setChatInputHistory = function(value)
		showHistoryWhenChatInput = value
	end
	WG['chat_filtered'].getChatInputHistory = function()
		return showHistoryWhenChatInput
	end
	WG['chat_filtered'].setInputButton = function(value)
		inputButton = value
	end
	WG['chat_filtered'].getHandleInput = function()
		return handleTextInput
	end
	WG['chat_filtered'].setHandleInput = function(value)
	...
	WG['chat_filtered'].getChatVolume = function()
		return sndChatFileVolume
	end
	WG['chat_filtered'].setChatVolume = function(value)
		sndChatFileVolume = value
	end
	WG['chat_filtered'].getBackgroundOpacity = function()
		return backgroundOpacity
	end
	WG['chat_filtered'].setBackgroundOpacity = function(value)
		backgroundOpacity = value
	end
	WG['chat_filtered'].getMaxLines = function()
		return maxLines
	end
	WG['chat_filtered'].setMaxLines = function(value)
		maxLines = value
		widget:ViewResize()
	end
	WG['chat_filtered'].getMaxConsoleLines = function()
		return maxLines
	end
	WG['chat_filtered'].setMaxConsoleLines = function(value)
		maxConsoleLines = value
		widget:ViewResize()
	end
	WG['chat_filtered'].getFontsize = function()
		return fontsizeMult
	end
	WG['chat_filtered'].setFontsize = function(value)
		fontsizeMult = value
		widget:ViewResize()
	end
	WG['chat_filtered'].addChatLine = function(gameFrame, lineType, name, nameText, text, orgLineID, ignore, chatLineID)
		addChatLine(gameFrame, lineType, name, nameText, text, orgLineID, ignore, chatLineID, true)
	end
	WG['chat_filtered'].addChatProcessor = function(id, func)
		if type(func) == 'function' then
			chatProcessors[id] = func
		end
	end
	WG['chat_filtered'].removeChatProcessor = function(id)
...
function widget:Shutdown()
	...
	WG['chat_filtered'] = nil
	if WG['guishader'] then
		WG['guishader'].RemoveRect('chat_filtered')
		WG['guishader'].RemoveRect('chatinput_filtered')
		WG['guishader'].RemoveRect('chatinputautocomplete_filtered')
	end
	if uiTex then
		gl.DeleteTexture(uiTex)
		uiTex = nil
